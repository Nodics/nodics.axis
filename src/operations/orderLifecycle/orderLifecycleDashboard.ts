...
function status(record: WorkbenchRecord): string {
  return scalarText(record.status ?? record.state).toUpperCase();
}

function requestType(record: WorkbenchRecord): string {
  return scalarText(
    record.requestType ?? record.type ?? record.lifecycleType,
  ).toUpperCase();
}
...
function queueCode(
...
): OrderLifecycleOperatorQueueCode | undefined {
  const value =
    `${navigation.id} ${navigation.label} ${navigation.route}`.toLowerCase();
  ...
  if (value.includes('cancellation') || value.includes('cancel'))
...
function requestTypes(
  navigation: AxisNavigationItem,
  code: OrderLifecycleOperatorQueueCode,
): readonly string[] {
 ...
 */
export function orderLifecycleOperatorQueues(
  navigation: readonly AxisNavigationItem[],
): readonly OrderLifecycleOperatorQueue[] {
  return navigation
    .map((item): OrderLifecycleOperatorQueue | undefined => {
      ...
      return {
        ...
        actionLabels: Object.freeze(
          (item.lifecycleActions ?? []).map((action) => action.label),
        ),
        summary:
          item.help?.summary ??
          item.workbenchPresentation?.fixedFilters?.[0]?.label ??
          item.label,
      };
    })
...
export function orderLifecycleDashboardItem(
  ...
  if (
    refundStatuses.has(currentStatus) ||
    hasEvidence(record, 'reconciliationRequired')
  ) {
    return {
      bucket: 'refundReconciliation',
      label: 'Refund reconciliation',
      urgent: true,
      recommendedActionIds: ['reconcile', 'approve-refund'].filter((id) =>
        actionIds.has(id),
      ),
    };
  }
  if (
    (type === 'RETURN' || type === 'EXCHANGE' || type === 'REPLACEMENT') &&
    (returnStatuses.has(currentStatus) || hasEvidence(record, 'rmaCode'))
  ) {
    return {

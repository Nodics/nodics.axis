...
export function GuidedInitializationWorkspace(props: Props) {
  ...
  if (props.profiles.length === 0)
    return (
      <Alert severity="info">
        No initialization profile is configured for this runtime.
      </Alert>
    );
  return (
    <Stack spacing={2}>
      ...
      {props.operationError ? (
        <Alert severity="error">{props.operationError}</Alert>
      ) : null}
          ...
          <Paper key={profile.profileCode} variant="outlined" sx={{ p: 2 }}>
            <Stack spacing={1.5}>
              ...
              {profile.steps.map((step) => (
                <Typography
                  key={`${profile.profileCode}:${step.order}`}
                  variant="body2"
                >
                  {step.order}. {typeCopy[step.dataType].label}: {step.releases.length}{' '}
                  release(s),{' '}
                  {
                    step.releases.filter((release) => release.status === 'CURRENT')
                      .length
                  }{' '}
                  current
                </Typography>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                <Button
                  disabled={props.operationPending || profile.blocked}
                  onClick={() => props.onRun(profile.profileCode, 'validate')}
                  variant="outlined"
                >
                  Validate plan
                </Button>
                <Button
                  disabled={
                    props.operationPending || profile.blocked || installable === 0
                  }
                  onClick={() => props.onRun(profile.profileCode, 'install')}
                  variant="contained"
                >
                  {props.operationPending ? 'Working…' : 'Validate and initialize'}
                </Button>
              </Stack>
            </Stack>
          </Paper>

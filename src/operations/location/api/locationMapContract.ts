...
export function categoryForFeature(
  feature: MapFeatureCategoryInput,
  presentation?: MapPresentation,
): MapCategory | undefined {
  if (!presentation) return undefined;
  const label = typeof feature.name === 'string' ? feature.name : feature.name?.en;
  const terms = [
    feature.collectionPointType,
    ...(feature.serviceCapabilities || []),
    label,
  ]
    .filter(Boolean)
    .join(' ')
  ...
  return (
    presentation.categories.find(
      (category) =>
        category.code !== presentation.defaultCategoryCode &&
        category.matchTerms.some((term) => terms.includes(term.toLowerCase())),
    ) ||
    presentation.categories.find(
      (category) => category.code === presentation.defaultCategoryCode,
    )
  );
}
/** Applies the same modifier rule to both renderer families without intercepting ordinary page scroll. */
export function shouldHandleMapWheel(
  event: Pick<WheelEvent, 'metaKey' | 'ctrlKey' | 'deltaY'>,
...
export function parseMapPresentation(value: unknown): MapPresentation | undefined {
  ...
  if (
    ...
    raw.categories.length > 12
  )
    return undefined;
  const codes = new Set<string>();
  for (const category of raw.categories as readonly MapCategory[]) {
    if (
      !category ||
      typeof category.code !== 'string' ||
      !/^[a-z][a-z0-9-]{0,47}$/.test(category.code) ||
      codes.has(category.code) ||
      typeof category.label !== 'string' ||
      !category.label.trim() ||
      !/^#[0-9a-f]{6}$/i.test(category.color) ||
      !Array.isArray(category.matchTerms) ||
      category.matchTerms.some(
...
export function parseMapInteraction(value: unknown): MapInteraction | undefined {
  ...
  const fields: [number, number, number][] = [
    [raw.wheelStep, 0.25, 3],
    [raw.wheelCooldownMs, 0, 1000],
    [raw.zoomAnimationSeconds, 0, 2],
  ];
  return fields.every(

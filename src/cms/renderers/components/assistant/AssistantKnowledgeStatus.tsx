...
export function AssistantKnowledgeStatus(props: AssistantKnowledgeStatusProps) {
  if (props.loading) {
    return <Stack direction="row" spacing={1} sx={{ p: 2 }}><CircularProgress size={18} /><Typography>{props.title}</Typography></Stack>;
  }
  if (props.error) return <Alert severity="warning">{props.unavailableLabel}: {props.error}</Alert>;
  if (!props.status) return null;
  const chunks = props.status.sources.reduce((total, source) => total + source.chunksProjected, 0);
  return (
    <Box sx={{ borderBottom: '1px solid', borderColor: 'divider', px: { xs: 2, md: 3 }, py: 1.5 }}>
      <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1.5} sx={{ alignItems: { lg: 'center' } }}>
        <Typography sx={{ fontWeight: 700 }}>{props.title}</Typography>
        <Chip size="small" label={`${props.sourcesLabel}: ${props.status.sources.length}`} />
        <Chip size="small" label={`${props.chunksLabel}: ${chunks}`} />
        {props.status.lastRefreshAt ? <Typography color="text.secondary" variant="caption">{props.lastRefreshLabel}: {new Date(props.status.lastRefreshAt).toLocaleString()}</Typography> : null}
        <Stack direction="row" spacing={1} sx={{ ml: { lg: 'auto' }, overflowX: 'auto' }}>
          {props.status.sources.map((source) => (
            ...
            >
              {props.refreshingSource === source.code ? props.refreshingLabel : `${props.refreshLabel} ${source.code}`}
            </Button>
          ))}
        </Stack>
      </Stack>
